...
_push() {
  docker push jstefanny/homebridge:$VERSION
}

_debug() {
  # Run (first time)
  echo "please go to /root and start run.sh"
  docker run -ti --entrypoint /bin/bash --net=host -p 51826:51826 -v /etc/homebridge:/root/.homebridge --name $IMAGE_NAME jstefanny/homebridge:$VERSION 
}
